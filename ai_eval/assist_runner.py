...
def build_messages(task, ex, hints):
    if task == "span":
        ...
        content = {
            "id": ex["id"],
            "text": ex["text"],
            "spacy_entities": hints["entities"],
            "spacy_relations": hints["relations"],
        }
    elif task == "bio":
        prompt = PROMPT_BIO
        content = {
...
def call_openai(
    ...
    try:
        ...
        if os.environ.get("AZURE_OPENAI_ENDPOINT"):
            ...
            openai.api_key = os.environ.get("AZURE_OPENAI_API_KEY") or os.environ.get(
                "OPENAI_API_KEY"
            )
...
def main():
    ...
    ap.add_argument(
        "--task", required=True, choices=["span", "rel", "bio", "structured"]
    )
    ...
    ap.add_argument(
        "--limit", type=int, default=None, help="Process at most N examples"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=2500,
        help="Truncate text to this many chars before sending",
    )
    ap.add_argument("--retries", type=int, default=3)
    ap.add_argument(
        "--fallback-spacy",
        action="store_true",
        help="If GPT JSON fails, emit spaCy-only prediction",
    )
    ...
    for ex in load_jsonl(args.gold):
        ...
        if (
            args.task in ("span", "rel", "structured")
            and args.max_chars
            and len(ex["text"]) > args.max_chars
        ):
            ex = dict(ex)
        ...
        if (
            args.spacy_model
            and args.task in ("span", "rel", "structured")
            and ex.get("text")
        ):
        ...
        if not obj:
            ...
            try:
                retry_msgs = messages + [
                    {
                        "role": "system",
                        "content": "Return ONLY VALID JSON for the required schema. No commentary, no code fences.",
                    }
                ]
                txt = call_openai(
                    retry_msgs,
    ...
    badlog.close()
    print(
        f"Wrote {n_done} predictions to {args.out} (processed={n_total}, skipped_seen={len(seen)})"
    )
