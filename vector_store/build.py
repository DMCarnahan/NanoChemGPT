...
def _load_texts(src: Path, limit: Optional[int], text_key: Optional[str]) -> tuple[List[str], List[str]]:
    ids, texts = [], []
...
def _build_faiss(emb: np.ndarray, metric: str, out_path: Path):
    try:
        ...
        raise RuntimeError("faiss-cpu not installed. Try: `pip install faiss-cpu`") from e
    dim = emb.shape[1]
    if metric.lower() == "ip":
...
def main(argv: Optional[List[str]] = None):
    ...
    p.add_argument("--src", required=True, type=Path, help="Input JSONL file (one record per line).")
    p.add_argument("--out", required=True, type=Path, help="Output FAISS index file, e.g. data/index.faiss")
    p.add_argument("--meta-out", type=Path, default=None, help="Optional meta JSON path (defaults to OUT with .meta.json)")
    ...
    if not texts:
        print(f"[build] No texts found in {args.src}. Check keys or file.", file=sys.stderr)
        sys.exit(2)
    ...
    if backend == "openai":
        ...
        print(f"[build] Unknown backend: {backend}", file=sys.stderr); sys.exit(3)

    dim = emb.shape[1]
    index = _build_faiss(emb, args.metric, args.out)
    meta = {"ids": ids, "text_key": args.text_key, "dim": dim, "metric": args.metric, "backend": backend, "model": model, "count": len(ids)}
    meta_out.parent.mkdir(parents=True, exist_ok=True)
    meta_out.write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"[build] Wrote FAISS index → {args.out} (ntotal={index.ntotal}, dim={dim}, metric={args.metric})")
