...
# ---------------- Boot log ----------------
print(
    "[vector_store.v2] EMBED_BACKEND =",
    os.getenv("EMBED_BACKEND", "st"),
    "| EMBED_MODEL =",
    os.getenv("EMBED_MODEL", ""),
    "| OPENAI_EMB  =",
    os.getenv("EMBED_OPENAI_MODEL", ""),
)
    ...
    if _index is None:
        ...
        if ipath.exists():
            try:
                ...
                _load_meta()
                print(
                    "[vector_store.v2] loaded FAISS index with ntotal=", _index.ntotal
                )
                return _index
            except Exception as e:
    ...
    if p.exists():
        try:
            ...
            if isinstance(payload, dict) and payload.get("schema") == SCHEMA_VERSION:
                ...
                with gzip.open(
                    INDEX_DIR / "meta.json.gz", "rt", encoding="utf-8"
                ) as f2:
                    _meta = json.load(f2)
        except Exception:
            _meta = []
...
METHOD_TAGS = {
    "sol gel": "builtin:sol-gel",
    "sol-gel": "builtin:sol-gel",
    "solid state": "builtin:solid-state",
    "solid-state": "builtin:solid-state",
    "hydrothermal": "builtin:hydrothermal",
    "cvd": "builtin:cvd",
    "ald": "builtin:ald",
    "coprecipitation": "builtin:coprecipitation",
    "co-precipitation": "builtin:coprecipitation",
...
def _route_query(q: str) -> Tuple[List[str], List[str]]:
    ...
    common = re.findall(
        r"\b(cobalt|palladium|nickel|alumina|perovskite|mof|graphene)\b", ql
    )
...
def _shortlist_indices(
    ...
    if must_tags:
        idxs = [
            i
            for i in idxs
            if any(str(metas[i].get("tag", "")).startswith(t) for t in must_tags)
        ]
    if must_keywords:
...
def _mmr_select(
    query_vec: np.ndarray, doc_vecs: np.ndarray, k: int, lam: float = 0.5
) -> List[int]:
...
# LLM reranker
def _llm_rerank(
    query: str, items: List[Tuple[int, str, str]]
    ...
    try:
        ...
        scores = {
            int(d["i"]): float(d["score"])
            for d in _json.loads(text)
            if "i" in d and "score" in d
        }
        ordered = sorted(
            range(len(sample)), key=lambda i: scores.get(i, 0.0), reverse=True
        )
...
def add_document(
    ...
    *,
    ...
    defer_embed: Optional[bool] = None,
) -> List[int]:
    """
    ...
    with _lock:
        ...
        print(
            f"[vector_store] indexed {len(chunks)} chunks (total {len(_meta)}) tag={tag} doc_id={doc_id}"
        )
    return idxs
...
def delete_by_tag(prefix: str) -> int:
    ...
    with _lock:
        ...
        if removed:
            ...
            _persist()
            print(
                f"[vector_store.v2] delete_by_tag('{prefix}') removed {removed} chunks"
            )
        return removed
...
def search(
    ...
    *,
    ...
    with _lock:
        ...
        cand_idxs = _shortlist_indices(
            _meta, must_tags or [], must_keywords or [], cap=200
        )
        if not cand_idxs:
        ...
        cand = [
            (
                cand_idxs[j],
                _meta[cand_idxs[j]].get("tag", "ctx"),
                texts[j],
                float(D[0][ii]),
            )
            for ii, j in enumerate(I[0])
        ]
        ...
        if cand_llm is not None and len(cand_llm) >= 1 and len(cand_llm) <= len(cand):
            ...
            cand = [
                (mid, tag, txt, lookup.get((mid, tag, txt), 0.0))
                for (mid, tag, txt) in cand_llm
            ]
        ...
        if return_structured:
            hits = [
                {"id": mid, "tag": tag, "score": score, "text": txt}
                for (mid, tag, txt, score) in cand
            ]
