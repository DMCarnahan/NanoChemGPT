...
def _read_json(p: Path) -> str:
    try:
        return json.dumps(json.loads(_read_text(p) or "{}"), ensure_ascii=False, indent=2)
    except Exception:
        return _read_text(p)

def _extract_text(p: Path) -> str:
    suf = p.suffix.lower()
    if suf in (".txt",".md",".rst",".log"): return _read_text(p)
    if suf == ".pdf": return _read_pdf(p)
    if suf == ".docx": return _read_docx(p)
    if suf in (".html",".htm"): return _read_html(p)
...
class _MiniTfidf:
    ...
    def _tf(tokens: List[str]) -> Dict[str,float]:
        tf: Dict[str,float] = {}
        for w in tokens: tf[w] = tf.get(w,0.0)+1.0
    ...
    def fit(self, docs: List["_Doc"]) -> None:
        ...
        self.doc_vectors=[]; self.doc_norms=[]
        for d in docs:
...
class UploadsVectorSearch:
    def __init__(self, docs: List[_Doc], backend_obj: Union[_MiniTfidf,_STBackend]) -> None:
        self._docs = docs
        ...
        **kwargs
        ...
        for p in sorted(set(paths)):
            try:
                ...
                if len(text) > 100_000: text = text[:100_000]
                docs.append(_Doc(path=p.resolve(), text=text, title=p.name,
        ...
        if use_st:
            try:
                ...
                be = _MiniTfidf(); be.fit(docs)  # type: ignore
            except Exception:
                be = _MiniTfidf(); be.fit(docs)  # type: ignore
        else:
            be = _MiniTfidf(); be.fit(docs)  # type: ignore
    ...
    def _snippet(self, i: int, query: str, radius: int = 160) -> str:
        ...
        for tok in _tokenize(query):
            j = t.find(tok)
            if j != -1: pos = j; break
        s = max(0, pos - radius); e = min(len(t), pos + radius)
    ...
    def search(self, query: str, top_k: int = 5, **kwargs) -> List[Dict[str, Union[str,float,dict]]]:
        if not query or not self._docs: return []
        # alias k -> top_k
        if 'k' in kwargs and (top_k is None or top_k == 5):
            try: top_k = int(kwargs['k'])
            except Exception: pass
        if not isinstance(top_k,int) or top_k <= 0: top_k = 5
