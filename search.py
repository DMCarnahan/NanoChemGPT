...
METHOD_TERMS = [
    ...
    "hot injection", "polyol", "thermal decomposition", "microemulsion",
...
def _cache_key(url: str, params: Dict[str, Any]) -> str:
    """Generate a cache key based on URL and sorted params."""
...
def parse_query_features(q: str) -> dict:
    """Extract materials, shapes, sizes, methods, ligands from query."""
    q_low = q.lower()
    mats = [canon for canon, syns in MATERIAL_SYNONYMS.items() if any(s in q_low for s in syns + [canon])]
    shapes = [canon for canon, syns in SHAPE_SYNONYMS.items() if any(s in q_low for s in syns + [canon])]
    methods = [t for t in METHOD_TERMS if t in q_low]
    ligands = [t for t in LIGAND_TERMS if t in q_low]
...
def build_expanded_query(original_q: str, feats: dict) -> str:
    """Expand query with synonyms and features."""
    parts = [original_q.strip()]
    if feats["materials"]:
        mat_tokens = [tok for m in feats["materials"] for tok in MATERIAL_SYNONYMS.get(m, [m])]
        parts.append(_or_group(mat_tokens))
    if feats["shapes"]:
        shape_tokens = [tok for s in feats["shapes"] for tok in SHAPE_SYNONYMS.get(s, [s])]
...
def _norm_crossref_item(x: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize Crossref item to standard dict."""
    ...
    venue = x.get("container-title", [""])[0] if x.get("container-title") else ""
    publisher = x.get("publisher") or ""
    venue_str = " • ".join(v for v in [venue, publisher] if v)
    return {
        "title": title, "year": yr, "venue": venue_str, "source": "Crossref",
        "doi": doi or None, "url": url, "abstract": ""
    }

def _request_with_cache(url: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """Request with cache support."""
...
def _journal_boost(venue: str) -> float:
    """Return boost for journal venue."""
    v = venue or ""
    return sum(float(w) for name, w in JOURNAL_BOOSTS.items() if name.lower() in v.lower())

def _score_item(q: str, feats: dict, it: Dict[str, Any]) -> float:
    """Score a search result item."""
...
def basic_search(q: str, n: int = 6) -> List[Dict[str, Any]]:
    """Perform a basic search and return top n results."""
    ...
    scored = [(s, it) for it in items if (s := _score_item(q, feats, it)) > -1e8]
