...
def fallback_methods_from_sections(
    sections: List[Dict[str, Any]], top_k=5, min_score=1.6
):
    cands = []
...
def main(in_path: str, out_path: str):
    ...
    with src.open("r", encoding="utf-8") as fin, dst.open(
        ...
        for line in fin:
            ...
            added += len(mp)
    print(
        f"Processed {n_in} records → {n_out}. Added methods paragraphs to {added} entries."
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python scripts\\bundle_add_fallback.py <in_bundle.jsonl> <out_bundle.jsonl>"
        )
        sys.exit(1)
