...
def test_transcribe_endpoint():
    ...
    try:
        ...
        if response1.status_code == 200:
            ...
            print("Extracted facts:", len(result1.get("extracted_facts", {}).get("materials", [])), "materials")
            print("Procedure steps:", len(result1.get("extracted_facts", {}).get("procedure", [])), "steps")
        else:
    ...
    try:
        ...
        if response2.status_code == 200:
            ...
            if "robot_conversion_error" in result2:
                print(f"⚠️ Robot conversion warning: {result2['robot_conversion_error']}")
        else:
    ...
    try:
        with open(temp_file_path, 'rb') as f:
            ...
            response3 = requests.post(f"{base_url}/ask", data=data, files=files, timeout=30)
            ...
            if response3.status_code == 200:
                ...
                if result3.get("ok"):
                    ...
                    if "method_paragraph_used" in result3:
                        print(f"Method paragraph extraction: {result3['method_paragraph_used']}")
                else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if processed_length < original_length:
                ...
                print("ℹ️ Full text used (method extraction may not have found a specific paragraph)")
