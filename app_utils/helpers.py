...
def judge_sufficiency(
    ...
    min_chars = (
        int(min_chars) if min_chars is not None else env_int("JUDGE_MIN_CHARS", 48)
    )
    min_score = (
        float(min_score) if min_score is not None else env_float("JUDGE_MIN_SCORE", 0.0)
    )
...
def judge_hits(hits, min_hits=1, min_score=0.0, min_chars=48):
    ...
    for h in hits or []:
        score = (
            ...
            if isinstance(h, dict)
            else float(getattr(h, "score", 0.0))
        )
        text = (
            h.get("text", "") if isinstance(h, dict) else getattr(h, "text", "")
        ) or ""
        if len(text) >= min_chars and score >= min_score:
            good += 1
    return good >= min_hits
