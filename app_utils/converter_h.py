...
DEFAULTS = {
    ...
    "base_addition_ph_range": (
        9.0,
        10.0,
    ),  # default pH if base addition lacks explicit pH
...
# ---------------- Regex ----------------
TIME_RX = re.compile(
    r"(?:This\s+should\s+take|for|over|approximately|about)\s*(?:approximately\s*)?(\d+(?:\.\d+)?)\s*(minutes?|hours?)",
    re.I,
)
PH_RX = re.compile(
    r"pH\s*(?:of(?:\s*around)?|around|=|≈|~)?\s*(\d+(?:\.\d+)?)\s*(?:-|–|—|\bto\b)\s*(\d+(?:\.\d+)?)",
    re.I,
)
...
# ---------------- Holding rules ----------------
def ensure_pickup_before_action(
    micro_ops: List[Dict[str, Any]] | None,
    ...
    for op in micro_ops:
        ...
        if verb == "place" and obj and _is_container(obj) and obj not in held:
            out.append(
                {"verb": "pick_up", "object": obj, "from": location.get(obj, "bench")}
            )
            held.add(obj)
        ...
        if verb == "pour":
            src = (
                obj
                if (obj and _is_container(obj) and op.get("from") is None)
                else (frm if _is_container(frm) else None)
            )
        ...
        if verb == "place" and obj:
            held.discard(obj)
            location[obj] = to or location.get(obj, "bench")
    return out


# ---------------- Stir idle auto-stop ----------------
def inject_stir_stop_if_idle(
    micro_ops: List[Dict[str, Any]] | None, lookahead: int
) -> List[Dict[str, Any]] | None:
    if not isinstance(micro_ops, list):
        return micro_ops
    out = []
        ...
        if isinstance(op, dict) and op.get("verb") == "wait":
            ...
            will_mix = any(
                ...
                and (
                    (
                        a.get("verb") == "set"
                        and a.get("device") == "stir_plate"
                        and a.get("param") == "rpm"
                    )
                    or (a.get("verb") == "pour")
                    or (a.get("verb") == "place" and a.get("to") == "stir_plate")
                )
                for a in ahead
            )
            if not will_mix:
                out.append(
                    {
                        ...
                        "param": "rpm",
                        "value": 0,
                        "note": "auto-stop to avoid idle spinning",
                    }
                )
    return out
    ...
    for seq_key in ("ops", "micro_ops"):
        ...
        for o in seq:
            if isinstance(o, dict) and (
                o.get("op") == "wait" or o.get("verb") == "wait"
            ):
                o["minutes"] = minutes
...
def _maybe_add_monitor_ph(step: Dict[str, Any], lo: float, hi: float) -> None:
    ...
    existing = sum(
        1
        for m in step["micro_ops"]
        if isinstance(m, dict) and m.get("verb") == "measure" and m.get("param") == "pH"
    )
    to_add = max(0, checks - existing)
    for _ in range(to_add):
        step["micro_ops"].append({"verb": "measure", "param": "pH"})
        step["micro_ops"].append(
            {"verb": "set", "param": "addition_rate", "value": "slow"}
        )
...
def augment_steps_with_time_ph_and_defaults(data: Dict[str, Any]) -> None:
    for step in data.get("steps", []) or []:
        ...
        if (
            minutes is None
            and action in {"add", "add_solvent", "transfer"}
            and ("dropwise" in raw.lower() or "slowly" in raw.lower())
        ):
        ...
        is_base_add = action in {"add", "add_solvent", "transfer"} and any(
            k in raw.lower()
            for k in ("naoh", "sodium hydroxide", "nh4oh", "ammonium hydroxide", "base")
        )
...
def _ensure_centrifuge_motion(step):
    ...
    # ensure rpm + power on
    new.append(
        {"verb": "set", "device": cfid, "param": "rpm", "value": rpm, "step_index": idx}
    )
    new.append(
        {
            "verb": "set",
...
def _sync_step_wait_minutes(step):
    ...
    for m in step.get("micro_ops") or []:
        if (
            m.get("verb") == "wait"
            and "minutes" in m
            and ("centrifuge" not in step.get("raw", ""))
            and not any(op.get("op") == "centrifuge" for op in step.get("ops", []))
        ):
            m = {**m, "minutes": minutes}
        new.append(m)
    step["micro_ops"] = new
...
def _unify_heating_placement(step):
    ...
    has_hp_temp = any(
        m.get("verb") == "set"
        and m.get("device") in ("HP1", "hotplate", "hotplate_id")
...
def apply_postprocessing(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(data.get("micro_plan"), list):
        ...
        data["micro_plan"] = inject_stir_stop_if_idle(
            data["micro_plan"], lookahead=lookahead
        )
    ...
    for step in data.get("steps", []) or []:
        if isinstance(step, dict):
            ...
            step["micro_ops"] = inject_stir_stop_if_idle(
                step["micro_ops"], lookahead=lookahead
            )
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="Postprocess a converter JSON with holding/time/pH/stir fixes."
    )
