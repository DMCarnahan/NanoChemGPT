...
def best_chunks_from_text(text: str, query: str, max_chunk_chars: int = 1200, top_k: int = 3) -> List[str]:
    if not text: return []
...
def pick_method_paragraph(text: str) -> str:
    ...
    cues = [
        ...
        r"\b5\s*m[lL]\s*/?\s*min\b", r"\b45\s*°\s*C\b", r"\b50\s*°\s*C\b",
        r"\b24\s*h\b", r"\bpH\s*12\b", r"\bautotitrator\b|\bdl50\b|\bmettler\b",
