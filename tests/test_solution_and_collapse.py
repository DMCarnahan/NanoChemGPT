...
def test_solution_flag_and_name_preserved():
    ...
    assert "sodium hydroxide" in struct.get("display_name", ""), "Full reagent name lost"
    assert struct.get("name").startswith("sodium hydroxide"), "Stripped name missing base chemical"
...
def test_duplicate_set_collapse_in_min_plan():
    ...
    sets = [a for a in doc.get("micro_plan_min", []) if a.get("verb") == "set" and a.get("param") == "temperature_C" and a.get("value") == 60]
    # Should only have one temperature_C set in minimal plan after collapse
    assert len(sets) == 1, f"Expected 1 collapsed temperature set, found {len(sets)}: {sets}"
