...
def test_freeform_drying_pickup_place():
    ...
    places = [
        ...
        if a.get("verb") == "place" and a.get("to") in {"OV1", "oven"}
    ]
    assert places, "No oven placement found"
    idx = places[0]
    assert idx > 0 and mp[idx - 1].get("verb") == "pick_up"
    assert any(
        a.get("verb") == "set" and a.get("device") in {"OV1", "oven"} for a in mp
    )
...
def test_autotitrator_variant_phrase():
    doc = convert_text_to_robot_ops(
        "Autotitrator-assisted addition: add solution A to vessel V2 at 2.5 mL/min while stirring for 5 minutes."
    )
    assert any(
        a.get("verb") == "set"
        and a.get("param") in {"rate_mL_per_min", "rate_ml_per_min"}
        and a.get("value") == 2.5
        for a in doc.get("micro_plan", [])
    )
