...
def test_nanorod_query_relevance_and_references(monkeypatch):
    ...
    def fake_retriever(query, k=8, level=None, **kwargs):
        return [{
            'meta': {'doi': '10.1000/sn2020', 'title': 'Synthesis of SnO nanorods with controlled aspect ratio', 'authors': ['Alice']},
    ...
    def fake_harvest(queries, use_grobid=None, jid=None):
        return [
            {'title': 'Hydrothermal growth of SnO nanorods', 'year': '2019', 'doi': '10.2000/sn_hydro', 'authors': ['Bob']}
        ]
    ...
    class FakeClient:
        class chat:
            class completions:
                @staticmethod
                def create(**kwargs):
                    return FakeResp("You can synthesize SnO nanorods via hydrothermal methods [1].\n\n## References\n[1] Synthesis of SnO nanorods with controlled aspect ratio (2018)")
    ...
    resp = client.post('/ask', json={'question': 'how can i synthesize diameter 10 nm length 50 nm SnO nanorods?', 'allow_fetch': True})
    ...
    assert any('nanorod' in t or 'sno' in t or 'sn' in t for t in titles), f"Unexpected refs: {titles}"
