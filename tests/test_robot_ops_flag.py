...
def test_robot_ops_flag_roundtrip(monkeypatch):
    ...
    class FakeClient:
        def __init__(self):
            self.chat = type(
                "chat",
                (),
                {"completions": type("c", (), {"create": FakeChat().create})},
            )()
    ...
    resp = client.post(
        "/ask?robot_ops=1",
        json={"question": "Give a cobalt nanowire synthesis protocol"},
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data.get("ok") is True
    assert (
        "robot_operations" in data
    ), f"robot_operations missing; keys={list(data.keys())}"
    assert "robot_rules" in data, "robot_rules alias missing"
    ops = data["robot_operations"]
    assert isinstance(
        ops, dict
    ), "robot_operations should be dict returned by convert_text_to_robot_ops"
