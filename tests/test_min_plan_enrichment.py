...
def test_oven_set_present_in_min_plan():
    ...
    if any("dry" in (s.get("raw", "").lower()) for s in doc.get("steps", [])):
        assert any(
            a.get("device") in {"OV1", "oven"} and a.get("param") == "temperature_C"
            for a in doc["micro_plan_min"]
        ), "oven temperature set missing in minimal plan"
...
def test_reagent_name_not_truncated():
    ...
    dissolve_step = next(
        (s for s in doc["steps"] if s.get("action") == "dissolve"), None
    )
    assert dissolve_step, "dissolve step missing"
    assert any(
        "FeSO4" in (r.get("name") if isinstance(r, dict) else r)
        for r in dissolve_step.get("reagents_structured", [])
    ), "FeSO4·7H2O name truncated"
