...
def test_robot_ops_fallback_basic():
    ...
    assert any(a in actions for a in ('stir','heat_hold','postprocess','add','add_solvent')), actions
    # Verify micro_plan created
    assert isinstance(doc.get('micro_plan'), list) and doc['micro_plan'], 'micro_plan should not be empty'
    # Ensure defaults inserted
    d = doc.get('defaults') or {}
    assert d.get('stir_rpm') and d.get('centrifuge_rpm'), 'Defaults missing required rpm values'
