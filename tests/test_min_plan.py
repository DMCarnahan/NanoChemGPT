...
def test_min_plan_delays_present():
    ...
    assert delays and any(
        d.get("minutes") == 30 for d in delays
    ), "30 min delay missing"
...
def test_min_plan_device_mapping():
    ...
    try:
        ...
        assert any(
            a.get("device") == "hotplate" for a in sets
        ), "Device mapping did not apply"
    finally:
