...
def test_pour_volume_units_augmented():
    ...
    doc["steps"][0]["ops"].insert(
        0, {"op": "add_solvent", "solvent": "ethanol", "volume": 10}
    )
    ...
    pours = [
        m
        for m in out["micro_plan"]
        if m.get("verb") == "pour" and m.get("volume") == 10
    ]
    assert (
        pours
    ), "Expected at least one pour with volume 10 generated from add_solvent op"
...
def test_provenance_collapsed_from_steps_sorted():
    ...
    set_ops = [
        m
        for m in out["micro_plan"]
        if m.get("verb") == "set" and m.get("param") == "temperature_C"
    ]
...
def test_executor_metadata_and_repairs_present():
    ...
    doc["micro_plan"] = [
        m for m in doc["micro_plan"] if m.get("verb") not in {"place", "pick_up"}
    ]
    out = apply_postprocessing(copy.deepcopy(doc))
    ...
    inserted = any(
        r.startswith("inserted_hotplate_pickup_place") for r in meta.get("repairs", [])
    )
    ...
    if not inserted:
        assert meta.get("repairs") == [] or all(
            isinstance(r, str) for r in meta.get("repairs")
        )
