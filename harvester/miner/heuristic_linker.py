...
def link_doc(doc):
    ...
    def sent_id_for_span(a: int, b: int) -> int:
        for i, (sa, sb) in enumerate(sent_bounds):
            if a >= sa and b <= sb:
                return i
        return (
            min(range(len(sent_bounds)), key=lambda i: abs(a - sent_bounds[i][0]))
            if sent_bounds
            else 0
        )
    ...
    for a in actions:
        ...
        op = LinkedOp(
            action_text=a.text, action_span=(a.start_char, a.end_char), sentence_id=si
        )
        ...
        for e in cand:
            ...
            if not field:
                continue
            if (
                abs(e.start_char - a.start_char) <= 60
                or sent_id_for_span(e.start_char, e.end_char) == si
            ):
