...
class EnhancedRelevanceFilter:
    ...
    def score_domain_specificity(
        self, journal: str, keywords: List[str] = None
    ) -> float:
        ...
        if not journal:
            ...
            if any(hj in journal_lower for hj in HIGH_IMPACT_JOURNALS):
                ...
                term in journal_lower for term in ["nano", "material", "chemistry"]
            ):
                journal_score = 0.7
            elif any(
                term in journal_lower for term in ["physics", "science", "nature"]
            ):
                journal_score = 0.5
            else:
                journal_score = 0.2
    ...
    def score_entity_relevance(
        self, entities: List[Dict], query_terms: Set[str] = None
    ) -> float:
        ...
        entity_score = (
            ...
            + len([e for e in entities if e.get("label") in ["TEMP", "TIME", "AMOUNT"]])
            * 0.3
        )
        ...
        if query_terms:
            ...
            query_matches = sum(
                1 for et in entity_texts if any(qt in et for qt in query_terms)
            )
    ...
    def calculate_relevance_score(
        self, paper: Dict, query_terms: Set[str] = None
    ) -> RelevanceScore:
        ...
        quality_score = (
            ...
            + (min(1.0, len(text.split()) / 5000) if text else 0.0)
            * 0.3  # Content completeness
            + (
                1.0
                if journal.lower() in [j.lower() for j in HIGH_IMPACT_JOURNALS]
                else 0.5
            )
            * 0.2
        )
    ...
    def filter_papers(
        self, papers: List[Dict], query_terms: Set[str] = None, top_k: int = None
...
def enhance_harvester_relevance(papers: List[Dict], config: Dict = None) -> List[Dict]:
    """
