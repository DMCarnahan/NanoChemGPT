...
class EnhancedCitationFilter:
    ...
    def extract_citation_context(
        self, text: str, citation_nums: List[int], window_size: int = 150
    ) -> Dict[int, str]:
    ...
    def score_query_alignment(
        self, citation_context: str, query: str, intent: str = "procedure"
    ) -> float:
        ...
        if intent == "procedure":
            ...
            if re.search(
                r"\b(compar|versus|better|superior|advantag)\b", context_lower
            ):
                intent_bonus += 0.3
    ...
    def score_content_quality(self, reference: Dict) -> float:
        ...
        if any(
            ...
            domain_term in journal
            for domain_term in ["nano", "material", "chemistry", "physics"]
        ):
            quality_score += 0.2
    ...
    def score_context_match(self, citation_context: str, response_text: str) -> float:
        ...
        response_terms = {
            t for t in response_terms if len(t) > 2 and t not in stopwords
        }
    ...
    def score_authority(self, reference: Dict, citation_count: int = 0) -> float:
        ...
        if "nature" in journal or "science" in journal:
            ...
            term in journal for term in ["advanced", "nano", "materials", "chemistry"]
        ):
            authority_score += 0.3
    ...
    def rank_citations(
    ...
    ) -> List[Tuple[int, CitationRelevance]]:
        ...
        for i, ref in enumerate(references, 1):
            ...
            query_alignment = self.score_query_alignment(
                citation_context, query, intent
            )
    ...
    def filter_low_relevance_citations(
        self,
        response_text: str,
        references: List[Dict],
        query: str,
        ...
        relevant_citations = [
            (num, rel) for num, rel in rankings if rel.total_score >= min_score
        ]
        ...
        filtered_response = re.sub(
            r"\[(\d+(?:,\s*\d+)*)\]", replace_citation, response_text
        )
...
def enhance_citation_relevance(
    response_text: str, references: List[Dict], query: str, intent: str = "procedure"
) -> Tuple[str, List[Dict]]:
    """
