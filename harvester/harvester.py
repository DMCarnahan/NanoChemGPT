...
from eupmc_api import fetch_fulltext_jats, search_eupmc
from grobid_client import pdf_to_tei
from jats_utils import filter_methods_sections as filt_jats
from jats_utils import jats_to_sections
from miner.runtime import get_miner
from oa_resolver import resolve_oa
from tei_utils import filter_methods_sections as filt_tei
from tei_utils import tei_to_sections
from tqdm import tqdm
from utils import ensure_dir, safe_slug, write_json
...
def _arxiv_id_from_any(url_or_id: str) -> str:
    ...
    if not s:
        return ""
    m = re.search(
        r"arxiv\.org/(?:abs|pdf)/([0-9]{4}\.[0-9]{4,5})(?:\.pdf)?", s, flags=re.I
    )
    if m:
        return m.group(1)
...
def _canon_rec(rec: dict) -> dict:
    ...
    if not arxiv_id:
        arxiv_id = _arxiv_id_from_any(
            (rec.get("id", "") + " " + rec.get("url", "")).strip()
        )
    ...
    return {
        ...
        "source": source
        or ("arxiv" if arxiv_id else ("eupmc" if pmcid else rec.get("source", ""))),
    }
...
TIME_RX = (
    r"(?:\b\d+(?:\.\d+)?\s?(?:s|sec|secs|seconds|min|mins|h|hr|hrs|hours|day|days)\b)"
)
SPEED_RX = r"(?:\b\d+(?:\.\d+)?\s?(?:rpm|r\.?\s?min(?:-1|⁻¹)?)\b)"
CONC_RX = r"(?:\b\d+(?:\.\d+)?\s?(?:M|mM|μM|µM|uM|%|wt%|vol%|v/v|w/w)\b)"

ACTION_RX = re.compile(
    r"\b(" + "|".join(map(re.escape, ACTION_WORDS)) + r")(?:ed|ing|s)?\b", re.I
)
...
_SECTION_RX = re.compile(
    ...
    r"(?=^\s*(results?|discussion|conclusion|acknowledg(e)?ments?)\b|\Z)",
    re.S | re.M,
)
...
def fallback_methods_from_sections(
    sections: list[dict], top_k: int = 5, min_score: float = 2.0
):
        ...
        try:
            ...
            for rec in dedup:
                paper_dict = {
                    ...
                    "year": rec.get("year"),
                    "doi": rec.get("doi"),
                    "isOpenAccess": rec.get("access_route")
                    not in ["unknown", "closed"],
            ...
            enhanced_papers = enhance_harvester_relevance(
                papers_for_filtering, relevance_config
            )
            ...
            for i, rec in enumerate(dedup):
                if i < len(enhanced_papers):
                    ...
                    rec["relevance_score"] = enhanced_papers[i].get(
                        "relevance_score", 0.0
                    )
                    rec["relevance_breakdown"] = enhanced_papers[i].get(
                        "relevance_breakdown", {}
                    )
                    rec["relevance_reasons"] = enhanced_papers[i].get(
                        "relevance_reasons", []
                    )
            ...
            logger.warning(
                f"Enhanced relevance filtering failed: {e}. Continuing with original papers."
            )

    else:
        logger.info(
            f"Enhanced relevance filtering disabled. Processing all {len(dedup)} papers."
        )
    ...
    with open(bundle, "w", encoding="utf-8") as fout:
        for rec in tqdm(dedup, desc="Processing"):
            ...
            if not methods:
                ...
                if rec.get("doi"):
                    try:
                        ...
                        if oa.get("is_oa"):
                            rec["pdf_url"] = (
                                rec.get("pdf_url") or oa.get("pdf_url") or oa.get("url")
                            )
                    except Exception:
                        pass
                    ...
                    if pdf_bytes and is_pdf_bytes(pdf_bytes):
                        ...
                        if USE_GROBID:
                            try:
                                ...
                                logger.warning(
                                    "[GROBID] Error; will try plain-text fallback: %s",
                                    e,
                                )
                        ...
                        if not methods:
                            try:
                                plain = extract_plain_text_from_pdf(
                                    pdf_bytes
                                )  # may not exist → NameError
                            except NameError:
                                plain = ""
                            except Exception as e:
            ...
            if not methods and (all_sections or plain):
                ...
                methods = fallback_methods_from_sections(
                    all_sections, top_k=8, min_score=1.2
                )
            ...
            if not methods:
                ...
                if meta_text:
                    all_sections = all_sections or [
                        {"heading": "metadata", "text": meta_text}
                    ]
                    methods = fallback_methods_from_sections(
                        all_sections, top_k=5, min_score=1.0
                    )
            ...
            if not paper.get("raw"):
                ...
                if all_sections:
                    raw_source = "\n\n".join(
                        sec.get("text", "") for sec in all_sections if sec.get("text")
                    )
                if not raw_source:
                    meta_bits = []
                    if rec.get("abstract"):
