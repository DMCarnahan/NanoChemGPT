...
def judge_sufficiency(hits, intent: str):
        ...
        norm.append(
            {
                ...
                **{
                    k: v
                    for k, v in d.items()
                    if k not in {"sim", "source_domain", "slots_present", "entity_hit"}
                },
            }
        )
    ...
    mean_topk = (
        clamp01(sum(_to_float(h.get("sim", 0.0), 0.0) for h in top) / k) if k else 0.0
    )
    evidence_count = sum((_to_float(h.get("sim", 0.0), 0.0)) >= SIM_FLOOR for h in top)
    distinct_sources = len(
        {(h.get("source_domain") or "") for h in top if h.get("source_domain")}
    )
    ...
    coverage = (
        (len(merged_slots & required) / float(len(required))) if required else 0.0
    )
    ...
    return (
        ...
        decision,
        {
            "reason": "threshold" if decision == "mine" else "ok",
            "top1": top1,
            "mean_topk": mean_topk,
            "coverage": coverage,
            "fresh_ok": fresh_ok,
            ...
            "evidence_count": evidence_count,
            "distinct_sources": distinct_sources,
            "threshold": THRESHOLD.get(intent, 0.5),
            "score": score,
        },
    )
