...
function renderRationaleWithCitations(text,refs){let html=escapeHtml(text||'');html=html.replace(/\[(\d+)\]/g,(m,n)=>{const idx=parseInt(n,10)-1;const r=Array.isArray(refs)?refs[idx]:null;const url=r&&r.url?r.url:(r&&r.doi?`https://doi.org/${r.doi}`:null);return url?`<a href="${url}" target="_blank" rel="noopener">[${n}]</a>`:m;});html=html.replace(/\[CTX\]/g,'<span class="badge badge-ctx">[CTX]</span>').replace(/\[GEN\]/g,'<span class="badge badge-gen">[GEN]</span>');return html;}
...
async function askQuestion(){hide(qs('#askMsg'));hide(qs('#jsonBlock'));const q=(qs('#questionInput').value||'').trim();if(!q){showAlert('#askMsg','error','Type a question first.');return;}const btn=qs('#askBtn');btn.disabled=true;btn.querySelector('.spinner')?.classList.remove('hidden');qs('#parseBtn').disabled=true;qs('#saveTxtBtn').disabled=true;try{const r=await fetch('/ask',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question:q})});if(!r.ok){const txt=await r.text();showAlert('#askMsg','error',`Ask failed (${r.status}): ${txt||'Unknown error'}`);return;}const data=await r.json();lastAnswer=data.answer||'';lastQuestion=q;qs('#answerPre').textContent=lastAnswer||'(empty)';qs('#rationalePre').innerHTML=renderRationaleWithCitations(data.rationale||'',data.references||[]);showRefs(data.references||[]);qs('#parseBtn').disabled=!lastAnswer;qs('#saveTxtBtn').disabled=!lastAnswer;showAlert('#askMsg','success','Answer ready.');}catch(err){showAlert('#askMsg','error',`Network error: ${err}`);}finally{btn.disabled=false;btn.querySelector('.spinner')?.classList.add('hidden');}}

function downloadFile(filename,text,mime='application/json'){const blob=new Blob([text],{type:mime});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);}
function safeName(s){return (s||'parsed').toLowerCase().replace(/[^a-z0-9\-_.]+/g,'-').replace(/-+/g,'-').slice(0,80);}

function showJsonPreview(pretty){const block=qs('#jsonBlock');if(!block)return;const want=qs('#showJsonPreview')?.checked;if(want){qs('#jsonPre').textContent=pretty;block.classList.remove('hidden');}else{block.classList.add('hidden');}}

async function parseAnswer(){const robot=!!qs('#robotMode')?.checked;let text=(typeof lastAnswer==='string'&&lastAnswer.trim())?lastAnswer:(qs('#answerPre')?.textContent||'').trim();hide(qs('#jsonBlock'));if(!text){showAlert('#askMsg','error','No answer to parse yet. Click “Ask” first.');return;}try{const r=await fetch('/parse',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text,robot})});const raw=await r.text();if(!r.ok){showAlert('#askMsg','error',`Parse failed (${r.status}): ${raw||'Unknown error'}`);return;}let obj;try{obj=JSON.parse(raw);}catch{qs('#jsonPre').textContent=raw;qs('#jsonBlock').classList.remove('hidden');showAlert('#askMsg','error','Server returned non‑JSON. Showing raw text.');return;}const pretty=JSON.stringify(obj,null,2);showJsonPreview(pretty);showAlert('#askMsg','success','Parsed to JSON (downloaded).');const base=safeName(lastQuestion||'answer');downloadFile(`${base||'answer'}.json`,pretty,'application/json');}catch(err){showAlert('#askMsg','error',`Network/JS error: ${err}`);}}

async function saveTxt(){if(!lastAnswer){showAlert('#askMsg','error','No answer to save yet.');return;}try{const r=await fetch('/save_txt',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({answer:lastAnswer,question:lastQuestion})});if(!r.ok){const txt=await r.text();showAlert('#askMsg','error',`Save failed (${r.status}): ${txt||'Unknown error'}`);return;}const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='answer.txt';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);showAlert('#askMsg','success','Saved as TXT.');}catch(err){showAlert('#askMsg','error',`Network error: ${err}`);}}
