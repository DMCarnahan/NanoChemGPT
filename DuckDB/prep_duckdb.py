...
def make_duckdb_from_parquet(parquet_path: str, duckdb_path: str, table: str) -> None:
    ...
    con.execute(f"CREATE OR REPLACE TABLE {table} AS SELECT * FROM read_parquet('{parq_sql}')")
    con.execute("CHECKPOINT")  # flush to disk
    con.close()
    print(f"[prep] wrote DB? {p_db.exists()} → {p_db}")

def main(argv=None):
    ap = argparse.ArgumentParser(description="Prepare Parquet and/or DuckDB from raw datasets.")
    ap.add_argument("--json-glob", default=None, help="Glob of JSON/JSONL files to convert")
    ...
    ap.add_argument("--from-parquet", default=None, help="Existing Parquet to import into DuckDB")
    ap.add_argument("--duckdb", default=None, help="Output DuckDB path to create/update")
    ap.add_argument("--table",  default="reactions", help="DuckDB table name (default reactions)")
    ...
    if parquet_src and args.duckdb:
        print(f"[prep] Parquet → DuckDB table '{args.table}': {parquet_src} -> {args.duckdb}")
