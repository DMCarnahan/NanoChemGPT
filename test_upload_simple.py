...
def simulate_pdf_processing(jid: str, filename: str):
    ...
    if not vs_available:
        ...
        _set_job(jid, status="done", warning="Job was stuck at indexing stage and auto-completed (vector store disabled)")
        ...
        if p.exists():
            ...
            if j.get("status") == "processing" and j.get("progress") == 100:
                ...
                if current_time - updated_at > 30:  # 30 seconds
                    print(f"🔧 Auto-completing stuck job {jid} after {current_time - updated_at:.1f}s")
                    j["status"] = "done"
                    ...
                    print(f"⏳ Job not stuck long enough yet ({current_time - updated_at:.1f}s < 30s)")
            else:
...
if __name__ == "__main__":
    ...
    thread = threading.Thread(target=simulate_pdf_processing, args=(test_jid, "test.pdf"))
    thread.start()
